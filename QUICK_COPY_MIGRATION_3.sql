...
CREATE TABLE commissions (
  ...
  listing_id UUID REFERENCES marketplace_listings(listing_id) ON DELETE SET NULL,
  buyer_id UUID,
  seller_id UUID,
...
ALTER TABLE rfqs ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMP;
...
ALTER TABLE parts ADD COLUMN IF NOT EXISTS created_by TEXT;
ALTER TABLE parts ADD COLUMN IF NOT EXISTS updated_by TEXT;
ALTER TABLE rfqs ADD COLUMN IF NOT EXISTS updated_by TEXT;

-- Add version tracking for key tables
ALTER TABLE parts ADD COLUMN IF NOT EXISTS version INTEGER DEFAULT 1;
...
LEFT JOIN vendor_quotes vq ON v.vendor_id = vq.vendor_id
LEFT JOIN orders o ON v.vendor_id = o.vendor_id AND o.status != 'cancelled'
...
FROM customers c
LEFT JOIN quotes q ON c.customer_id::TEXT = q.customer_id
LEFT JOIN orders o ON c.customer_id = o.customer_id
LEFT JOIN customer_equipment e ON c.customer_id = e.customer_id
