...
-- Original: 001_create_tables.sql
-- All tables prefixed with 'ace_' to avoid conflicts
-- Generated: 2025-12-07T14:46:26.675Z

-- Ace Electric Parts System - Database Schema
...
CREATE TABLE ace_part_cross_references (
...
-- RFQ-Vendor relationships (which vendors received which RFQs)
CREATE TABLE ace_rfq_vendors (
  rfq_vendor_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  rfq_id UUID REFERENCES ace_rfqs(rfq_id) ON DELETE CASCADE,
  vendor_id UUID REFERENCES ace_vendors(vendor_id) ON DELETE CASCADE,
...
-- Vendor quotes table
CREATE TABLE ace_vendor_quotes (
  quote_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  rfq_vendor_id UUID REFERENCES ace_rfq_vendors(rfq_vendor_id) ON DELETE CASCADE,
  vendor_id UUID REFERENCES ace_vendors(vendor_id) ON DELETE CASCADE,
...
-- Contacts table (trade shows, LinkedIn, etc.)
CREATE TABLE ace_contacts (
...
-- Jobs table (shop and field service)
CREATE TABLE ace_jobs (
...
-- Quotes table (shop, field service, sales)
CREATE TABLE ace_quotes (
  quote_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
-- Repairs table
CREATE TABLE ace_repairs (
  repair_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  job_id UUID REFERENCES ace_jobs(job_id) ON DELETE SET NULL,
...
-- Equipment table (customer equipment)
CREATE TABLE ace_equipment (
...
CREATE TABLE ace_training_courses (
...
-- Scraping jobs table
CREATE TABLE ace_scraping_jobs (
  job_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX idx_vendors_discovered_via ON vendors(discovered_via);
CREATE INDEX idx_rfqs_status ON rfqs(status);
CREATE INDEX idx_rfqs_created_date ON rfqs(created_date);
CREATE INDEX idx_quotes_status ON quotes(status);
CREATE INDEX idx_repairs_warranty_expires ON repairs(warranty_expires_at);
