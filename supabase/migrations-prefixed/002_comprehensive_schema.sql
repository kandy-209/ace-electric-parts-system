...
-- Original: 002_comprehensive_schema.sql
-- All tables prefixed with 'ace_' to avoid conflicts
-- Generated: 2025-12-07T14:46:26.989Z
...
ALTER TABLE ace_parts ADD COLUMN IF NOT EXISTS search_vector tsvector;
ALTER TABLE ace_parts ADD COLUMN IF NOT EXISTS ml_features JSONB DEFAULT '{}'::jsonb;
ALTER TABLE ace_parts ADD COLUMN IF NOT EXISTS popularity_score DECIMAL(5, 2) DEFAULT 0;
ALTER TABLE ace_parts ADD COLUMN IF NOT EXISTS last_quoted_date TIMESTAMP;
ALTER TABLE ace_parts ADD COLUMN IF NOT EXISTS quote_count INTEGER DEFAULT 0;
ALTER TABLE ace_parts ADD COLUMN IF NOT EXISTS order_count INTEGER DEFAULT 0;

-- Enhanced vendors table
ALTER TABLE ace_vendors ADD COLUMN IF NOT EXISTS search_vector tsvector;
ALTER TABLE ace_vendors ADD COLUMN IF NOT EXISTS ml_features JSONB DEFAULT '{}'::jsonb;
ALTER TABLE ace_vendors ADD COLUMN IF NOT EXISTS response_rate DECIMAL(5, 2);
ALTER TABLE ace_vendors ADD COLUMN IF NOT EXISTS average_quote_time_hours DECIMAL(5, 2);
ALTER TABLE ace_vendors ADD COLUMN IF NOT EXISTS win_rate DECIMAL(5, 2);
ALTER TABLE ace_vendors ADD COLUMN IF NOT EXISTS total_quotes INTEGER DEFAULT 0;
ALTER TABLE ace_vendors ADD COLUMN IF NOT EXISTS total_orders INTEGER DEFAULT 0;
ALTER TABLE ace_vendors ADD COLUMN IF NOT EXISTS total_revenue DECIMAL(12, 2) DEFAULT 0;
ALTER TABLE ace_vendors ADD COLUMN IF NOT EXISTS linkedin_url TEXT;
ALTER TABLE ace_vendors ADD COLUMN IF NOT EXISTS portal_user_id UUID REFERENCES auth.users(id);

-- Enhanced RFQs table
ALTER TABLE ace_rfqs ADD COLUMN IF NOT EXISTS customer_id TEXT;
ALTER TABLE ace_rfqs ADD COLUMN IF NOT EXISTS customer_name TEXT;
ALTER TABLE ace_rfqs ADD COLUMN IF NOT EXISTS sales_rep_id UUID;
ALTER TABLE ace_rfqs ADD COLUMN IF NOT EXISTS priority TEXT CHECK (priority IN ('low', 'medium', 'high', 'urgent')) DEFAULT 'medium';
ALTER TABLE ace_rfqs ADD COLUMN IF NOT EXISTS estimated_value DECIMAL(12, 2);
ALTER TABLE ace_rfqs ADD COLUMN IF NOT EXISTS awarded_vendor_id UUID REFERENCES ace_vendors(vendor_id);
ALTER TABLE ace_rfqs ADD COLUMN IF NOT EXISTS awarded_quote_id UUID;
ALTER TABLE ace_rfqs ADD COLUMN IF NOT EXISTS notes TEXT;
...
CREATE TABLE IF NOT EXISTS ace_orders (
  ...
  rfq_id UUID REFERENCES ace_rfqs(rfq_id),
  quote_id UUID,
  vendor_id UUID NOT NULL REFERENCES ace_vendors(vendor_id),
...
CREATE TABLE IF NOT EXISTS ace_customers (
...
CREATE TABLE IF NOT EXISTS ace_sales_reps (
  ...
  team_id UUID,
  manager_id UUID REFERENCES ace_sales_reps(rep_id),
...
CREATE TABLE IF NOT EXISTS ace_sales_teams (
  team_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  team_name TEXT NOT NULL,
  team_lead_id UUID REFERENCES ace_sales_reps(rep_id),
  description TEXT,
  created_at TIMESTAMP DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS ace_marketplace_deals (
  deal_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  deal_number TEXT UNIQUE NOT NULL,
  
  -- Parties
  buyer_id UUID REFERENCES ace_customers(customer_id),
  seller_vendor_id UUID REFERENCES ace_vendors(vendor_id),
  part_id UUID REFERENCES ace_parts(part_id),
...
CREATE TABLE IF NOT EXISTS ace_buyer_intents (
  intent_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  customer_id UUID REFERENCES ace_customers(customer_id),
  part_description TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS ace_seller_listings (
...
CREATE TABLE IF NOT EXISTS ace_communications (
  ...
  parent_communication_id UUID REFERENCES ace_communications(communication_id),
  template_id UUID,
...
CREATE TABLE IF NOT EXISTS ace_communication_templates (
...
CREATE TABLE IF NOT EXISTS ace_sales_notes (
  note_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  
  -- Related entities
  customer_id UUID REFERENCES ace_customers(customer_id),
  vendor_id UUID REFERENCES ace_vendors(vendor_id),
  rfq_id UUID REFERENCES ace_rfqs(rfq_id),
  order_id UUID REFERENCES ace_orders(order_id),
  
  -- Note details
  sales_rep_id UUID NOT NULL REFERENCES ace_sales_reps(rep_id),
...
CREATE TABLE IF NOT EXISTS ace_sales_activities (
...
CREATE TABLE IF NOT EXISTS ace_commissions (
  commission_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  
  -- Related entities
  sales_rep_id UUID NOT NULL REFERENCES ace_sales_reps(rep_id),
  order_id UUID REFERENCES ace_orders(order_id),
  deal_id UUID REFERENCES ace_marketplace_deals(deal_id),
  ...
  approved_by UUID REFERENCES ace_sales_reps(rep_id),
...
CREATE TABLE IF NOT EXISTS ace_analytics_events (
...
CREATE TABLE IF NOT EXISTS ace_performance_metrics (
...
CREATE TABLE IF NOT EXISTS ace_ml_models (
...
CREATE TABLE IF NOT EXISTS ace_ml_predictions (
  prediction_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  model_id UUID NOT NULL REFERENCES ace_ml_models(model_id),
...
CREATE TABLE IF NOT EXISTS ace_ml_training_jobs (
  job_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  model_id UUID REFERENCES ace_ml_models(model_id),
...
CREATE TABLE IF NOT EXISTS ace_integrations (
...
CREATE TABLE IF NOT EXISTS ace_sync_jobs (
  sync_job_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  integration_id UUID NOT NULL REFERENCES ace_integrations(integration_id),
...
CREATE TABLE IF NOT EXISTS ace_portal_users (
...
CREATE TABLE IF NOT EXISTS ace_contact_interactions (
  interaction_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  contact_id UUID REFERENCES ace_contacts(contact_id),
  vendor_id UUID REFERENCES ace_vendors(vendor_id),
...
BEGIN
  ...
  LEFT JOIN ace_vendor_quotes vq ON rv.rfq_vendor_id = vq.rfq_vendor_id
  LEFT JOIN ace_rfqs r ON r.awarded_vendor_id = v.vendor_id AND r.rfq_id = rv.rfq_id
  LEFT JOIN ace_orders o ON o.vendor_id = v.vendor_id
...
FROM ace_rfqs r
LEFT JOIN ace_rfq_vendors rv ON r.rfq_id = rv.rfq_id
LEFT JOIN ace_vendor_quotes vq ON rv.rfq_vendor_id = vq.rfq_vendor_id
...
LEFT JOIN ace_rfq_vendors rv ON v.vendor_id = rv.vendor_id
LEFT JOIN ace_vendor_quotes vq ON rv.rfq_vendor_id = vq.rfq_vendor_id
LEFT JOIN ace_rfqs r ON r.rfq_id = rv.rfq_id
LEFT JOIN ace_orders o ON o.vendor_id = v.vendor_id
GROUP BY v.vendor_id;
...
-- ALTER TABLE ace_customers ENABLE ROW LEVEL SECURITY;
-- ALTER TABLE ace_orders ENABLE ROW LEVEL SECURITY;
-- ALTER TABLE ace_sales_notes ENABLE ROW LEVEL SECURITY;
